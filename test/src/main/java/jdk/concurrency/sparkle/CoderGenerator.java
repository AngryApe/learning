/**
 * @Summary: defines the method for generating the code for all the objects.
...
public final class CoderGenerator {
    ...
    public static String getOrganizationCode(char nodeID) {
        String tmp = CoderGenerator
                ._getCode(nodeID, EnumSystemPlatform.CONST_SYSTEM_PLATFORM_USER.getIntValue(),
    ...
    public static String getRoleCode(char nodeID) {
        String tmp = CoderGenerator
                ._getCode(nodeID, EnumSystemPlatform.CONST_SYSTEM_PLATFORM_PRIVILEGE.getIntValue(),
    ...
    public static String getStationCode(char nodeID) {
        String tmp = CoderGenerator
                ._getCode(nodeID, EnumSystemPlatform.CONST_SYSTEM_PLATFORM_BIZ.getIntValue(),
    ...
    private static String _getCode(char nodeID, int type, int bizType) {
        ...
        long sequence = SparkleWrapper.getSequenceInMilliSecond(); //sequence in current time
        //        long lngTmp = Long.valueOf(String.valueOf(lngTime) + String.format("%05d",sequence)) ;//get the unique code in the format of @Long
        long lngTmp = lngTime + sequence;
        System.out.println("_getCode:"+ lngTime + ' ' + sequence);
        ...
        for (i = 0; i < CoderGenerator.CONST_CODE_LENGTH - tmp.length()
                - CoderGenerator.CONST_SYSTEM_LENGTH - CoderGenerator.CONST_BIZ_LENGTH - 1; i++) {
            tmp = '0' + tmp;
        }
